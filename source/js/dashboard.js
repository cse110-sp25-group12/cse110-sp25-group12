document.addEventListener('DOMContentLoaded', () => {
    ...
    // === DOM References ===
    const totalApplicationsEl = document.querySelector('.content-card:nth-child(1) .content-card-stat');
    const interviewsScheduledEl = document.querySelector('.content-card:nth-child(2) .content-card-stat');
    const activeProcessesEl = document.querySelector('.content-card:nth-child(3) .content-card-stat');
    const offersReceivedEl = document.querySelector('.content-card:nth-child(4) .content-card-stat');
    ...
    interviewsScheduledEl.textContent = applications.filter(app => app.status === 'Interviewing').length;
    activeProcessesEl.textContent = applications.filter(app =>
        ['Applied', 'Screening', 'Interviewing'].includes(app.status)
    ).length;
    offersReceivedEl.textContent = applications.filter(app => app.status === 'Offer').length;
    ...
    const chartColors = {
      ...
      onSurfaceVariant: getCSSColor('--md-sys-color-on-surface-variant') || '#6b7280',
      pieSliceColors: [
        '#4f46e5', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#9ca3af'
      ]
    };
    ...
    const statusLabels = ['Applied', 'Screening', 'Interviewing', 'Offer', 'Rejected', 'Ghosted'];
    const statusCounts = {
      Applied: 0,
      Screening: 0,
      Interviewing: 0,
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        const confirmReset = confirm('Are you sure you want to clear all application data?');
        if (confirmReset) {
